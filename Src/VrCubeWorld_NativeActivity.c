...
//#define OVR_SUCCESS(ovrInputState)
...
static void ovrScene_Create(ovrScene* scene, bool useMultiview) {
    ...
    for (int i = 0; i < NUM_ROTATIONS; i++) {
        ...
        ALOGV("Rotation x %f", scene->Rotations[i].x);
...
static ovrLayerProjection2 ovrRenderer_RenderFrame(
    ...
    //ovrTracking2 updatedTracking2 = *tracking;
    //double x_val = updatedTracking2.HeadPose.LinearAcceleration.x;
    //double time = updatedTracking2.HeadPose.TimeInSeconds;
    //ALOGV("Acceleration x %f", x_val);
    //ALOGV("Time in second %f", time);
...
static void ovrApp_Clear(ovrApp* app) {
    ...
    /***********************/
//    app->state = NULL;
//    app->remoteDeviceID = 0;
    /**********************/
    //figure out how to clear the array
    app->FrameIndex = 1;
    app->DisplayTime = 0;
    app->SwapInterval = 1;
    app->CpuLevel = 2;
...
static void ovrApp_HandleVrModeChanges(ovrApp* app) {
    if (app->Resumed != false && app->NativeWindow != NULL) {
        ...
        if (app->Ovr != NULL) {
            ...
            ALOGV("        eglGetCurren tSurface( EGL_DRAW ) = %p", eglGetCurrentSurface(EGL_DRAW));
        }
    }
}
...
void *inc_x(void *x_void_ptr)
{
    ...
    double x_gyro = tracking2.HeadPose.AngularVelocity.x;
    double y_gyro = tracking2.HeadPose.AngularVelocity.y;
    double z_gyro = tracking2.HeadPose.AngularVelocity.z;

    ALOGV("Accleration %f %f %f %f", current_time, x_acc, y_acc, z_acc);
 ...
 */
    ...
    ovrApp* appState = (ovrApp*) x_void_ptr;
/* increment x to 100 */
    int *x_ptr = (int *)x_void_ptr;

    double gap = 0.001;
    //double gap = 0.00071428;
    double prev = GetTimeInSeconds();
    while(1) {
        double current_time = GetTimeInSeconds();
        ...
        //ovrTracking2 tracking2 = vrapi_GetPredictedTracking2(appState->Ovr, current_time);
        ovrTracking tracking2 = vrapi_GetPredictedTracking(appState->Ovr, 0.0);
        double x_acc = tracking2.HeadPose.LinearAcceleration.x;
        double y_acc = tracking2.HeadPose.LinearAcceleration.y;
        double z_acc = tracking2.HeadPose.LinearAcceleration.z;
        ...
        ovrTrackingState tracking3 = ovr_GetTrackingState(session, 0.0, ovrFalse);
        ///Left controller data
        ovrInputState stateCon;
        double stateLeft1 = stateCon.Thumbstick[0].x;
        double stateLeft2 = stateCon.Thumbstick[0].y;
        double stateRight1 = stateCon.Thumbstick[1].x;
        double stateRight2 = stateCon.Thumbstick[1].y;
        ///You don't need two  separate variables for touches; it should return all touches no matter
        ///the controller
        double touches = stateCon.Touches;

        double x_acc1 = tracking2.HandPoses[0].LinearAcceleration.x;
        double y_acc1 = tracking2.HandPoses[0].LinearAcceleration.y;
        double z_acc1 = tracking2.HandPoses[0].LinearAcceleration.z;

        double x_gyro1 = tracking2.HandPoses[0].AngularAcceleration.x;
        double y_gyro1 = tracking2.HandPoses[0].AngularAcceleration.y;
        double z_gyro1 = tracking2.HandPoses[0].AngularAcceleration.z;

        ///Right controller data
        double x_acc2 = tracking2.HandPoses[1].LinearAcceleration.x;
        double y_acc2 = tracking2.HandPoses[1].LinearAcceleration.y;
        double z_acc2 = tracking2.HandPoses[1].LinearAcceleration.z; //Get some data

        double x_gyro2 = tracking2.HandPoses[1].AngularAcceleration.x;
        double y_gyro2 = tracking2.HandPoses[1].AngularAcceleration.y;
        double z_gyro2 = tracking2.HandPoses[1].AngularAcceleration.z;

        ALOGV("Acceleration(left controllers) %f %f %f %f", current_time, x_acc1, y_acc1, z_acc1);
        ALOGV("Gyroscope(left controllers)  %f %f %f %f", current_time, x_gyro1, y_gyro1, z_gyro1);
        ALOGV("Acceleration(right controllers) %f %f %f %f", current_time, x_acc2, y_acc2, z_acc2);
        ALOGV("Gyroscope(right controllers)  %f %f %f %f", current_time, x_gyro2, y_gyro2, z_gyro2);
        ALOGV("Thumbstick values(left): %f %f", stateLeft1, stateLeft2);
        ALOGV("Thumbstick values(right): %f %f", stateRight1, stateRight2);
        ALOGV("Touches values: %f", touches);

        /*******************************************************************************************/

//        ovrHandPoseHeader handHeader;
//        handHeader.Version = 1.17.2;
        //ovrInputState
...
//        if(vrapi_GetCurrentInputState(appState->Ovr, appState->remoteDeviceID, appState->state.Header) {
//
//        }
//        ovrInputStateTrackedRemote* state;
//        ovrDeviceID remoteDeviceID;
//        ovrInputStateTrackedRemote input;
////        input.Header.ControllerType = ovrControllerType_TrackedRemote;
////
////        ovrControllerType touchController[] = {ovrControllerType_Reserved0, ovrControllerType_Reserved1};
////        //ovrInputStateHeader inputHeader;
////        int t;
////        //if (controllerType & ovrControllerType_Reserved0) s+= "LTouch";
////
////        //inputHeader.ControllerType = touchController[t];
////
////        ovrInputCapabilityHeader inputHeader;
////
////        ovrDeviceID deviceID = inputHeader.DeviceID;
////
////        ovrResult result1 = vrapi_GetInputTrackingState(appState->Ovr, appState->remoteDeviceID, current_time, &tracking2);
////        ovrResult result = vrapi_GetCurrentInputState(appState->Ovr, appState->remoteDeviceID, &input.Header);
////        if (result >= 0) {
////            //get the vector coordinates for trackpadposition
////            //as much data from inputstatetrackedremote
////            //maybe if statement is not needed.
////            //double x_acc1 =
////            continue;
////        }
//        //ovrResult result2 = vrapi_GetInputTrackingState(appState->Ovr, appState->remoteDeviceID, current_time, tracking2);
////
////
//        double x_acc1 = input.TrackpadPosition.x;
//        double y_acc1 = input.TrackpadPosition.y;
//
//        double trigGrip = input.GripTrigger;
//        double trigIndex = input.IndexTrigger;
//
//        //<ovrTouch>input.Touches;
//
////        ALOGV("Acceleration (Controllers): %f %f", x_acc1, y_acc1);
////        ALOGV("Capacitive sensors (Grip & Index): %f %f", trigGrip, trigIndex);
//        if (input.Header.ControllerType == ovrControllerType_Gamepad) {
//            ALOGV("This is a test. %d", input.Header.ControllerType);
//        }
////        ovrDeviceID leftID = (uint32_t)dc36774d072f3cbd;
////        ovrInputStateTrackedRemote remote[2] = {leftID, rightID};
////
////
////        for(size_t t = 0; t < 2; ++t) {
////            ovrControllerType touchController[] = {ovrControllerType_Reserved0, ovrControllerType_Reserved1};
//////            ovrInputStateTrackedRemote* remote = appState->state;
////
////
////        }
////
//        ovrDeviceID deviceID[2];
//        ovrInputStateTrackedRemote left, right;
//        ovrTracking leftTrack, rightTrack;
//        ovrInputCapabilityHeader inputCap;
////        if (tracking2) {
//
//        for (int i = 0; ; i++) {
//           ovrResult result = vrapi_EnumerateInputDevices(appState->Ovr, i, &inputCap);
//            if(inputCap.Type == ovrControllerType_TrackedRemote) {
//                ovrTracking remoteTrack;
//                ovrInputTrackedRemoteCapabilities remoteCap;
//                remoteCap.Header = inputCap;
//                if (vrapi_GetInputDeviceCapabilities(appState->Ovr, &remoteCap.Header)) {
//                    ovrInputStateTrackedRemote inputState;
//                    inputState.Header.ControllerType = ovrControllerType_TrackedRemote;
//                    if (vrapi_GetCurrentInputState(appState->Ovr, inputCap.DeviceID, &inputState.Header) >= 0) {
//                        if (vrapi_GetInputTrackingState(appState->Ovr, inputCap.DeviceID, current_time, &remoteTrack)) {
//                            if (remoteCap.ControllerCapabilities & ovrControllerType_Reserved1) {
//                                right = inputState;
//                                rightTrack = remoteTrack;
//                                deviceID[1] = inputCap.DeviceID;
//                            } else {
//                                left = inputState;
//                                leftTrack = remoteTrack;
//                                deviceID[0] = inputCap.DeviceID;
//                            }
//                        }
//                    }
//                }
//
//            }
//            ALOGV("This is a test. Result: %d", result);
//        }
...
void android_main(struct android_app* app) {
    ...
    // Create a thread to record the accelerometer and gyroscope data
    pthread_t inc_x_thread;
    int x=0;
    // pthread_create(&inc_x_thread, NULL, inc_x, (void *) &x);
    pthread_create(&inc_x_thread, NULL, inc_x, (void *) &appState);
